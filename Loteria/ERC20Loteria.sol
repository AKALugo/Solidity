...
interface IERC20 {
    ...
    //Devuelve un bool resultado de la operacion indicada
    function transferLoteria(address emiter, address recipient, uint256 numTokens) external returns(bool);
...
contract ERC20Basic is IERC20 {
    ...
    function transferLoteria(address emiter, address recipient, uint256 numTokens) public override returns(bool) {
        require (numTokens <= balances[emiter]);

        balances[emiter] = balances[emiter].sub(numTokens); 
        balances[recipient] = balances[recipient].add(numTokens);

        emit Transfer(emiter, recipient, numTokens);
